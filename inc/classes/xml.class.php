...
class TTreexmlExport
{
   ...
   var $filePath;
     ...
     function TTreexmlExport($TreeObj,$startNodeId,$branch_obj_types,$outputFilePath)
     {
             
             $this->XMLparser = new XML();
             $this->data=$TreeObj->ExportNodes($startNodeId,$branch_obj_types);
     ...
     function process()
     {
         ...
         if(!empty($this->data))
            ...
            foreach ($this->data as $ExNode)
                ...
                $node=$this->XMLparser->createElement('tnode');
                $node->attributes=array('basic'=>$ExNode['basic'],
                                        'uniq'=>$ExNode['uniq'],
                                        'obj_type'=>$ExNode['obj_type'],
                                        );
                
                $this->XMLparser->appendChild($node);

                
                $this->WriteParamPack($id, $ExNode['params']);

                if (is_array($this->EXPcache[$ExNode['id']]))
                    {
                    $this->CopyNodes($id, $ExNode['id'], null, false);
                    }
                }
