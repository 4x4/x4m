...
function dhtmlXWindows() {
	...
	*/
	...
	this.skinParams = { // standard
			    ...
			    "dhx_skyblue"	: { "header_height": 29, "border_left_width": 0, "border_right_width": 0, "border_bottom_height": 0 }
...
//#}	
	...
	*/
	this.createWindow = function(id, x, y, width, height) {
		...
		*   @type: public
		*/
		win.bringToTop = function(val) {
			that._bringOnTop(this,val);
		...
		*/
		...
		win.onmousemove = function(e) {
			...
			if ((!this._allowResize) || (this._allowResizeGlobal == false)) {
				if (targetObj.style.cursor != "") targetObj.style.cursor = "";
...
//#}	
	...
	*/
	...
	this._bringOnTop = function(win,val) 
    {
		var cZIndex = win.zi;
		if(!val)
        {
            ...
            topZIndex =val;
            win.tzi=val;
            this.tzi= this._getTopZIndex(win._isSticked);
...
//#wind_buttons:09062008{
		...
		btnMinMax1.isVisible = false;
		btnMinMax1._isEnabled = false;
        btnMinMax1.style.display = "none";  
        ...
        if(!win.states)
        {
            ...
            btnClose._doOnClick = function() { that._hideWindow(win);}
        }
...
//#}
	...
	this._setWindowModal = function(win, state) {
		...
		if (state == true) {
            ...
            vp=this._get_page_size();  
            
            this.modalCoverD.style.height=vp[1]+'px';
		} else {
			this.modalWin = null;
			win._isModal = false;
			//
...
dhtmlXWindows.prototype._dhx_Engine = function() {
	...
	this._engineSetWindowBody = function(win) {
		...
		if (this.skin == "dhx_skyblue") {
			win.dhxContGlobal.obj._offsetWidth = 0;
			win.dhxContGlobal.obj._offsetHeight = 0;
			win.dhxContGlobal.obj._offsetLeft = 0;
