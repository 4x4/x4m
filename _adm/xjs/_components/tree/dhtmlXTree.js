...
function dhtmlXTreeObject(htmlObject,width,height,rootId,imgpath){
 ...
 this.imageArray=new Array("page.gif","folderOpen.gif","folderClosed.gif");
...
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode,objtype){
 ...
 if((this.globalNodeStorage[i].unParsed)&&(!skipParsing))
{
 ...
 var z=this.XMLLoader.getXMLTopNode("t");
 var i=0;
 var k=0;
...
{
 var zNode=this.findXML(z.childNodes[i],"i",itemId);
 if(zNode!==false)
{
 var nArr=new Array();
 while(1){
 nArr[nArr.length]=zNode.getAttribute("i");
 z=this._globalIdStorageFind(zNode.getAttribute("i"),true,true,true);
 if(z)break;
 ...
 dhtmlXTreeObject.prototype._drawNewTr=function(htmlObject,node)
{
 ...
 dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,objtype,image3,optionStr,childs,beforeNode,afterNode){
 ...
 switch(tempStr[i])
{
 ...
 Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,itemActionHandler,1,objtype);
...
*/
 ...
 dhtmlXTreeObject.prototype.insertNewItem=function(parentId,itemId,itemText,itemActionHandler,image1,objtype,optionStr,childs){
 var parentObject=this._globalIdStorageFind(parentId);
 if(!parentObject)return(-1);
 var nodez=this._attachChildNode(parentObject,itemId,itemText,itemActionHandler,image1,objtype,null,optionStr,childs);
 if((!this.XMLloadingWarning)&&(this.childCalc))this._fixChildCountLabel(parentObject);
 return nodez;
};

 dhtmlXTreeObject.prototype.insertNewChild=function(parentId,itemId,itemText,itemActionHandler,image1,objtype,optionStr,childs){
 return this.insertNewItem(parentId,itemId,itemText,itemActionHandler,image1,objtype,optionStr,childs);
}
 ...
 var nodeAskingCall="";
 if(!node){
 node=xml_obj.getXMLTopNode("t");
 parentId=node.getAttribute("i");
 //if(node.getAttribute("radio"))dhtmlObject.htmlNode._r_logic=true;
 ...
 for(var i=start||0;i<node.childNodes.length;i++)
{
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "n"))
{
 temp.XMLload=1;
...
// var sColor=nodx.getAttribute("sCol");
    ...
    var objtype = nodx.getAttribute("o");  
    
    if(typeof  dhtmlObject.ObjImages!='undefined')
    {       
     
        im0 =  dhtmlObject.ObjImages[objtype];
    }else{
        im0 = null;
    }
    
    var chd = (nodx.getAttribute("p")!="f") ? 1 : 0;  
...
/* if(achecked==-1)zST+=",HCHECKED";
 ...
 //картинки не передаются  все из кеша
 
 if(dhtmlObject.waitUpdateXML)
 //!
 var newNode=dhtmlObject.updateItem(cId,name,im0,im1,im2,achecked);
 else
 var newNode=dhtmlObject._attachChildNode(temp,cId,name,0,im0,objtype,null,null,chd,0,preNode);
 ...
 if((!dhtmlObject.slowParse)||(dhtmlObject.waitUpdateXML))
{
 ...
 if(node.childNodes[i].childNodes[a].tagName=="n")
 ...
 for(var i=1;i<=itemObject.childsCount;i++)
{
 ...
 dhtmlXTreeObject.prototype.getSelectedItemId=function(asarray)
{
 var str=new Array();
 for(var i=0;i<this._selected.length;i++)str[i]=this._selected[i].id;
 
 if(convertStringToBoolean(asarray))
 {
    return str;
 }else{
 ...
 else
{
 ...
 inp.checked=0;inp.src=this.imPath+this.checkArray[0];inp.style.width="15px";inp.style.height="15px";
 ...
 else
{
 ...
 td2.style.fontSize="8pt";td2.style.cursor=this.style_pointer;
...
{
 if(temp.childNodes[i].tagName=="t")
{
 if(!z)z=temp.childNodes[i].getAttribute("i");
 else z+=this.dlmtr+temp.childNodes[i].getAttribute("i");
}
}
 return z;
}
 ...
 else
{
 ...
 return this._getAllChecked("","",1).split(",");
}
...
dhtmlXTreeObject.prototype._createDragNode=function(htmlObject,e){
 ...
 if(this._itim_dg)
 for(var i=0;i<this._selected.length;i++)
 text[i]="<span><img style='position:relative;top:4px;' width='14px' height='14px' src='"+this._selected[i].span.parentNode.previousSibling.childNodes[0].src+"'></span>"+this._selected[i].span.innerHTML;
 else
 ...
 if(itemNode.childsCount!=0)
{
 ...
 //X3M
dhtmlXTreeObject.prototype.getNodeParam=function(node,param)
{
 var temp=this._globalIdStorageFind(node);  
 return  temp[param];
 }

dhtmlXTreeObject.prototype.setInheritanceArr = function(arr){
    this.inheritanceArr=arr;
}

dhtmlXTreeObject.prototype.setObjImages = function(obj){
    this.ObjImages=obj;
}



dhtmlXTreeObject.prototype.get_objtype=function(itemId){
    var sNode = this._globalIdStorageFind(itemId);  
    return sNode.objtype;
    }