...
function fileQueued(file) {
    try {
        ...
        progress.setStatus(_lang_matrix['waiting']);
...
function fileQueueError(file, errorCode, message) {
    try {
        if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
            alert("Слишком много файлов для закачки.\n" + (message === 0 ? "Вы достигли предела по рамеру файлов." : "Интервал размеров " + (message > 1 ? "до " + message + " файлов." : "1 файл.")));
            return;
        }
      ...
      progress.toggleCancel(false);
        ...
        case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
            progress.setStatus("Рамзер файла 0 байт!.");
            this.debug("Error Code: Zero byte file, File name: " + file.name + ", File size: " + file.size + ", Message: " + message);
            break;
        case SWFUpload.QUEUE_ERROR.INVALID_FILETYPE:
            progress.setStatus("Неправильный тип файла");
...
function uploadStart(file) {
    try {
        ...
        var progress = new FileProgress(file, this.customSettings.progressTarget);
        progress.setStatus(_lang_matrix['upload']);
        progress.toggleCancel(true, this);
    }
    catch (ex) {}
...
function uploadSuccess(file, serverData) {
    try {
        ...
        progress.setStatus(_lang_matrix['ready']);
        progress.toggleCancel(false);
        this.startUpload();
        xFileManager.refresh();
    } catch (ex) {
        this.debug(ex);
    }
}
...
function uploadError(file, errorCode, message) {
    try {
        ...
        case SWFUpload.UPLOAD_ERROR.IO_ERROR:
            progress.setStatus("Ошибка связи с сервером (IO) ");
            this.debug("Error Code: IO Error, File name: " + file.name + ", Message: " + message);
            break;
        case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
            progress.setStatus(_lang_matrix['security_error']);
...
function uploadComplete(file) {
    if (this.getStats().files_queued === 0) {
        ...
        Effect.Fade('sfu-upload',{afterFinish:function(){
              $('xmview').style.width='100%';  
        }})
    }
}
