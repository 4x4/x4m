...
dragContext.prototype._autoOpenItem=function(e,gridObject)
{  
       gridObject.refreshItem(gridObject._autoOpenId);
}
    ...
    if (this.target=="treeGrid")
    {
      if (this.dropmode=="child"){
          ...
          //this.tobj.openItem(this.tid);
                              this._autoOpenTimer=window.setTimeout(new callerFunction(this._autoOpenItem,this.tobj),1400);
                              this.tobj._autoOpenId=this.tid;   
      }
      else
        ...
        if (c.source=="tree")
        {
            ...
            if ((c.source=="treeGrid")){
            	...
            	if (c.sobj==c.tobj) new_row._xml=c.sobj.rowsAr[c.sid]._xml;
                                    new_row._attrs=c.sobj.rowsAr[c.sid]._attrs;
...
{                              
                  ...
                  {    
                     //    this._autoOpenTimer=window.setTimeout(new callerFunction(this._autoOpenItem,this),10);
...
{       
    ...
    var gridRowId = itemObject.idd;
    var treeNodeId = gridRowId;
        targetId=targetObject._attrs['id'];
   ...
   var img=(this.isTreeGrid()?this.getItemImage(gridRowId):"")
    ...
    if(this.gridToTreeElement(tree,treeNodeId,gridRowId,targetId)==-1)return;
