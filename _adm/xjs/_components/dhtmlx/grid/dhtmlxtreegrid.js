...
dhtmlXGridObject.prototype.refreshItem=function(id)
{
   // debugger;
    var r=this._h2.get[id];
   
   if(r) {
    this.deleteChildItems(r.id);
            if (this.callEvent("onDynXLS",[r.id]))
            {
                if(this.json_dataset)
                {
                    this.json_dataset.parent=r.id;
                    this._process_tree_json(this.json_dataset);    
                    this.json_dataset=null;
                }
...
eXcell_tree.prototype = new eXcell;
   ...
   eXcell_tree.prototype.setValueA = function(content){
	...
	eXcell_tree.prototype.setValue = function(valAr){
		...
		row.image=row.image||(row.buff._attrs["image"]||"leaf.gif");
		row.label=valAr;
...
dhtmlXGridObject.prototype._filterTreeA=function(column,value){ 
	...
	var add_line=function(el,s,t){
			...
			if (!t){
				t={id:el.id, _xml_await:el._xml_await, childs:[], level:el.level, parent:z, index:z.childs.length, image:el.image, state:el.state, buff:el.buff, has_kids:el.has_kids};
			z.childs.push(t);
			temp.get[t.id]=t;
			}
			return t;
