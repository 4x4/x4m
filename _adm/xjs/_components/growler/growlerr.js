...
k.Growler = Class.create({
		...
		this.growler.setStyle({ position: ((IE==6)?"absolute":"fixed"), padding: "10px", "width": opt.width, "z-index": "50000" });
			...
			switch(opt.location){
				...
				case "bc":
					...
					break;
				default:
					this.growler.setStyle({top: 0, right: 0, margin:'20px 10px 0 0'});
					break;
			}
	...
	,warn: function(msg, options){
		return specialNotice(this.growler, msg, options, _lang_common['warning'], "#F6BD6F", "#000");
	}
	,error: function(msg, options){
		return specialNotice(this.growler, msg, options, _lang_common['error'], "#F66F82", "#000");
	}
	,info: function(msg, options){
		return specialNotice(this.growler, msg, options, _lang_common['info'], "#BBF66F", "#000");
	}
