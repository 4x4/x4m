		...
		return {
			...
			onOk : function()
			{
				...
				if ( this.imageEditMode )
				{
                    ...
                    url=this.getValueOf( 'info', 'txtUrl');
                    rMatch  =  url.match(imgrex );    // Check value
                    if(rMatch)url=rMatch[2];
                        height=this.getValueOf( 'info', 'txtHeight');
                        width=this.getValueOf( 'info', 'txtWidth');
                        this.setValueOf( 'info', 'txtUrl', '/image.php/'+url+'?width='+width+'&height='+height+'&image='+url);
							...
							[
								{
									...
									[
										...
										},
										{
                                            ...
                                            onClick:function(t){
                                                el=t.data.dialog.getContentElement('info','txtUrl').getInputElement().getId();
                                                xFileManager.selectImage(el);
