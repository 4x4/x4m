...
XTRbanners.prototype =Object.extend(new _modulePrototype(),
    ...
    initialize: function()
        ...
        this.current_bannersgroup = null;
        this.current_bannersgroupname = null;
    ...
    tree_object_clicked: function(itemid)
        ...
        switch (object_type)
            ...
            case "_BANNERSGROUP":
                ...
                this.current_bannersgroup = itemid;
                this.current_bannersgroupname = this.tree.getItemText(itemid);
   ...
   create_banner: function()
        {            
            ...
            if (this.current_bannersgroup){
                xoad.html.importForm('abanner', {category : this.current_bannersgroup});
            }
            if (banner_type == 'gif') {
                
               var Xpop = new XTRpop('startXlistPage', null, { position: 'bottom', delta_x: -203, delta_y: 0, className : 'dialogtable02', donotdestroy:true });
               var xlist = this.group_dialog(Xpop.tool_tip.id,'load_xlist_link_pages',1, 'page', 'pageId','dialogtable2');
   ...
   save_banner:function()
    ...
    show_edit_banner: function(itemId)
        {    
            ...
            this.connector.execute({load_banner:{banner_id:itemId},get_banner_stats:{banner_id:itemId}});            
                ...
                xoad.html.importForm('stat',this.connector.result.banner_data);
				...
				if(this.connector.result.banner_data.banner_type == 'gif') {
				   if(this.connector.result.banner_data.use_page) {
					   $('link_block').hide();
					   $('int_page').show();
				   } else {
					   $('int_page').hide();
    ...
    {    
		var use_page_input = $('use_page');
        var use_page=false;
		if (use_page_input) {
			use_page=$('use_page').checked;
		}
        ...
        if ($('banner_type').value!='html' ){
            if($('file_name').value=='' && !use_page) {alert(_lang_banners['load_banner_file']);return void(0);}
        }
            ...
            get_action_properties:function(_action)
            {
                   ...
                   if(this.connector.result.xlist)
                   {
                     ...
                     _images=$H({
                       ...
                       {
                        resultSource:'showbannersName',
                        resultIDSource:'bannersId',
                        columnsHeaders:columns,
                        tableId:'dialogtable',
                        columnsHeadersWidth:_columnsHeadersWidth,
                        columnsInterpretAs:_columnsInterpretAs,
