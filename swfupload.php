...
require_once($_PATH['PATH_XOAD'].'classes/ConvertCharset.class.php');   
	...
	if (isset($_POST["PHPSESSID"])) {
        ...
        if (!isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) {
            ...
            $_FILES["Filedata"]["name"] = translit($_FILES["Filedata"]["name"]);
        }
...
// Settings
	$save_path = getcwd() . "/media/uploads/";				// The path were we will save the file (getcwd() may not be reliable and should be tested in your environment)
	$upload_name = "Filedata";
	$max_file_size_in_bytes = 2147483647;				// 2GB in bytes
        if(isset($_POST["file_types"])){
            $extension_whitelist = explode(",", $_POST["file_types"]);
	        //$extension_whitelist = array("doc", "xls", "txt", "rtf", "jpg", "gif", "png", "pdf");	// Allowed file extensions
        } else {
	...
	$is_valid_extension = false;
        if($extension_whitelist){
	        foreach ($extension_whitelist as $extension){
		        if (strcasecmp($file_extension, $extension) == 0){$is_valid_extension = true; break;
            }
        }
	}   
